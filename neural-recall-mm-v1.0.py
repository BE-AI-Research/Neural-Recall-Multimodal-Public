"""
title: Neural Recall
author: BE AI Research
author_url: https://github.com/BE-AI-Research
funding_url: https://checkout.square.site/merchant/G2KSH0GWPA83X/checkout/XFC5AWNQ3EF4DA2F3BB27KQ2
...
to compare vs project goals and create this function. Claude Opus 4.7 was used to extend the capabilities into vision. All of which are listed below, thank you all!
...
When the user message includes one or more images, examine each image carefully and extract any memorable facts that fit the categories above. Be thorough: images frequently carry details that are never mentioned in the accompanying text. Specifically look for:
- People in the scene: by default treat any person as "a person" and do not assume they are the user. Only attribute features to the user when the accompanying text identifies them as the user (e.g., "here's me at..."). If you visually recognize a public or notable figure, you may name them; otherwise describe them in generic terms (apparent age range, clothing, role).
- Possessions: vehicles, pets, devices, clothing, books, art, instruments, tools, jewelry. Note specific brand or model names ONLY when they are clearly readable in the image.
- Environment: room or place type, indoor vs. outdoor, lighting/time-of-day cues, season, visible landmarks, and geographic or cultural clues (signage language, architecture, license plates).
- Visible text: signs, labels, screenshots, documents, receipts, tattoos, name tags, logos. This is often the highest-signal content in an image; transcribe or summarize it when relevant.
- Activities: what is happening in the scene (cooking, exercising, traveling, working, etc.).
- Notable details: anything else clearly visible that fits an identity / behavior / preference / goal / relationship / possession category.

Confidence rule for images: if you cannot identify a detail with reasonable confidence, either omit it entirely or describe it in more general terms (e.g., "a dark sedan" rather than guessing the make and model; "a textbook on a desk" rather than guessing the title from a blurred cover). Do not fabricate details and do not speculate beyond what is clearly visible. It is better to record a less specific true fact than a confident guess.

Skip incidental background, blurry or unclear elements, and details that are clearly unrelated to the user (e.g., random strangers in a crowd, generic decor).
...
The current user message may include one or more images. When images are present, treat their visual content as part of the message context (people, objects, locations, visible text, activities) when scoring memory relevance.
...
Your output must be valid JSON only. No additional text.""",
        ...
        try:
            if "messages" in body and body["messages"]:
                ...
                if user_messages:
                    ...
                    # Split multimodal content so images can be passed to a
                    # vision-capable model alongside the text.
                    user_text, user_images = self._extract_message_parts(
    ...
    async def outlet(
        ...
        try:
            if "messages" in body and len(body["messages"]) >= 2:
                ...
                user_messages = [m for m in body["messages"] if m["role"] == "user"]
                if user_messages:
                    # Split multimodal content so images flow through to the
                    # vision-capable LLM alongside the text.
                    user_message, user_images = self._extract_message_parts(
                        user_messages[-1]["content"]
                    )
    ...
    def _extract_message_parts(self, content: Any) -> Tuple[str, List[str]]:
        """Split a chat message's content into plain text and image URLs.

        Open WebUI passes `content` as a string for text-only messages or
        as an OpenAI-style list of parts (with `type: "text"` and
        `type: "image_url"` items) for multimodal messages. Image URLs are
        returned in their original form (typically `data:image/...;base64,...`)
        so each backend can format them as needed.
        """
    ...
    async def _process_user_memories(
        ...
        memories = await self.identify_memories(
            user_message, existing_memories, user_timezone, images=images
        )
...
"""
            ...
            response = await self.query_llm_with_retry(
                system_prompt, input_text, images=images
            )
...
Current datetime: {current_datetime.strftime('%A, %B %d, %Y %H:%M:%S')} ({current_datetime.tzinfo})"""
    ...
    async def _query_ollama(
        ...
        if images:
            # Ollama's /api/chat expects raw base64 strings in `images`
            # (no data URL prefix). Strip any `data:...;base64,` prefix.
            ollama_images: List[str] = []
            for url_item in images:
                if not isinstance(url_item, str):
                    continue
                if url_item.startswith("data:"):
                    comma = url_item.find(",")
                    if comma >= 0:
                        ollama_images.append(url_item[comma + 1 :])
                else:
                    # Bare base64 or http URL - Ollama only handles base64,
                    # but pass through and let the server respond.
                    ollama_images.append(url_item)
            if ollama_images:
    ...
    async def _query_openai(
        ...
        if images:
            # OpenAI-compatible vision: content becomes a list of typed
            # parts. Pass URLs through as-is (typically data URLs).
            user_content: List[Dict[str, Any]] = [{"type": "text", "text": user_prompt}]
            for url_item in images:
                if isinstance(url_item, str) and url_item:
                    user_content.append(
